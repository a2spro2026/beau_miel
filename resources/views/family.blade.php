...
@section('title', $family->titre.' — BEAUMIEL')

@section('content')
<section class="family-page" aria-labelledby="familyPageTitle">
    <div class="container family-page-inner">
        <header class="family-page-head">
            ...
            <div class="family-page-hero">
                <img src="{{ $family->photoUrl() }}" alt="{{ $family->titre }}" class="family-page-cover">
                <div>
                    <p class="section-ornament-text">Même catégorie · mesures différentes</p>
                    <h1 class="section-title" id="familyPageTitle">{{ $family->titre }}</h1>
                    @if ($family->description)
                        <p class="section-subtitle">{{ $family->description }}</p>
                    @endif
                </div>
            </div>
        </header>
        ...
        <div class="shop-page-body">
            <div class="shop-measures">
                <p class="shop-col-title">Produits &amp; mesures</p>
                    ...
                    @foreach ($family->items as $item)
                        <article class="shop-item">
                            ...
                            <div class="shop-item-info">
                                ...
                                <span>Mesure : {{ $item->mesure }}</span>
                                <span class="shop-item-price">{{ number_format((float) $item->prix_u, 2, '.', '') }}</span>
                            </div>
                            <button
                                type="button"
                                class="shop-cart-btn"
            ...
            <aside class="shop-ticket" aria-label="Ticket commande">
                ...
                <div id="shopTicketLines" class="shop-ticket-lines">
                    <p class="shop-ticket-empty">Sélectionnez des mesures via l’icône panier.</p>
                </div>
...
<script>
    (() => {
        ...
        const renderTicket = () => {
            ...
            if (cart.size === 0) {
                ticket.innerHTML = '<p class="shop-ticket-empty">Sélectionnez des mesures via l’icône panier.</p>';
            }
